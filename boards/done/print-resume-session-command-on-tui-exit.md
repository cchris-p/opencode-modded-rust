...
- Extended `opencode_tui::TuiExit` so a normal exit carries the active session id (`TuiExit::Exit { session_id }`); dropped `Copy` in favor of `Clone`.
- `App::run` now reads the session id from the current route at exit (`crates/opencode-tui/src/app/app.rs`).
- `run_tui` in the CLI matches on `TuiExit` and prints a resume hint for normal exits; the detach path and its `opencode attach <url>` hint are unchanged (`crates/opencode-cli/src/main.rs`).
- The hint prefers `ort` when `OPENCODE_RUST_REPO` is present (the personal launcher exports it and forwards args to the Rust binary), otherwise falls back to `opencode`.
