...
## Reference source (re-pin pending)

- The frozen TypeScript reference line in `AGENTS.md` is `$HOME/repos/opencode-modded` at commit
  `e62912b5d18b73316c7bfd6e894b040698f6c880`.
- That commit is **no longer reachable** from `https://github.com/cchris-p/opencode-modded` (only
  `dev` and `experimental` are advertised; the GitHub API returns 422 for the SHA), and
  `$HOME/repos/opencode-modded` is not present on this machine.
- For this gate, evidence was taken from the closest reachable state: `origin/dev` at
  `e207624c48159b03dbe17dbc8e51bbcf23e72df5` (package version `1.18.29`, 2026-09-06), checked out
  sparsely under `$HOME/repos/opencode-modded`.
- The frozen line must be re-pinned or the checkout restored before the gate is considered passing;
  every line reference below must be re-verified against whatever line is finally used.

## Vanilla behavior to target (reference `e207624c4`)
...
- Created 2026-09-21. `$HOME/repos/opencode-modded` was absent and the pinned commit unreachable, so
  evidence was taken from the closest reachable `origin/dev` (`e207624c4`); re-pin before passing.
