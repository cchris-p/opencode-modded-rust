...
- Add per-tool-call expansion state, keyed by the tool call id, in `SessionView`, mirroring `expanded_reasoning` (`crates/opencode-tui/src/components/session.rs:41`, initialized `:59`, retained to visible ids `:868-869`). Session-local, not persisted.
- Give block-style tool calls a click hit region, reusing the `ThinkingToggleHit` pattern (`session.rs:30-33`): a new hit type recording `line_index` plus tool call id, a `tool_toggle_hits: Vec<...>` list cleared at the start of each render (parallel to `thinking_toggle_hits.clear()` at `session.rs:539`), populated in the tool-call render loop (`session.rs:740-782`). To do this cleanly, `render_tool_call` must report which output lines belong to the call, or expose an expandable/`collapsible` signal like `render_reasoning_part` does (`session.rs:706-734`).
- Extend `render_tool_call` (`session_tool.rs:58`) to accept the per-call expanded flag and render the full output when expanded, instead of capping at `preview_limit` (`session_tool.rs:149-165`). Collapsed behavior stays the current preview; expanded shows all `result_text` lines.
...
- Make the toggle clickable from the block header and from the more-lines affordance, following `handle_click` (`session.rs:906-934`) and the left-button dispatch at `app.rs:677-689`. A successful hit toggles the id's expanded state and returns `true` so the click is not also treated as a text selection start.
- Preserve existing semantics and layout: the completed-and-`show_tool_details`-off early return (`session_tool.rs:67-69`), the error branch (`session_tool.rs:111-132`), inline (non-block) tools, non-bash block tools, selection, scrolling, and thinking toggles.
- Allow full error output to be expanded too (error branch currently shows one line plus up to two more when details are on, `session_tool.rs:113-132`).
...
- `FEAT-028` Add a hide-tool-calls toggle that compacts runs to a tool-call count - same tool-call render surface (`session.rs:740-782`, `session_tool.rs`); the two features must not conflict.
...
- The current truncation is entirely inside `render_tool_call` (`session_tool.rs:133-166`); expansion belongs there, while hit-region bookkeeping and toggle state belong one level up in `session.rs`, mirroring how `render_reasoning_part` returns a `collapsible` signal that `session.rs` turns into hits (`session.rs:706-734`).
...
- The mouse path is already wired for session clicks (`app.rs:677-689`); the new hits only need to be consulted alongside `thinking_toggle_hits` in `handle_click`, and the click must be consumed so it does not also seed a selection (`app.rs:691-692`).
- `thinking_toggle_hits` is cleared at `session.rs:539` before the message loop; `tool_toggle_hits` should be cleared in the same pass and line indices must stay consistent with the `Paragraph` scroll used at `session.rs:882-890`.
- Keep the `expanded_reasoning.retain(...)` pruning pattern in mind (`session.rs:868-869`) so expansion state for calls no longer present is dropped; the analogous pruning applies to expanded tool calls.
