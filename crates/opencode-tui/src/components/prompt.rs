    ...
    fn history_next(&mut self) {
        ...
        if idx + 1 < self.history.len() {
            ...
            match self.history_draft.take() {
                ...
                _ => {
                    // No pre-browse draft to restore: keep the newest entry
                    // visible instead of clearing the box. Stay parked on the
                    // newest index so further Down presses are inert until the
                    // user edits, clears, or recalls.
...
mod tests {
    ...
    #[test]
    fn history_next_from_recall_back_to_draft_then_repeat_is_inert() {
        with_isolated_prompt(|mut prompt| {
            prompt.set_input("alpha".to_string());
            let _ = prompt.take_input();
            prompt.set_input("beta".to_string());
    ...
    fn history_next_without_recall_is_noop() {
        with_isolated_prompt(|mut prompt| {
            ...
            prompt.set_input("beta".to_string());
            let _ = prompt.take_input();
            prompt.set_input("keep".to_string());

            prompt.history_next_entry();
            assert_eq!(prompt.get_input(), "keep");
        });
    }
