...
impl Provider for VercelProvider {
    ...
    async fn chat_stream(&self, request: ChatRequest) -> Result<StreamResult, ProviderError> {
        ...
        let stream = crate::stream::sse_event_stream(response.bytes_stream(), vercel_line_events);
...
fn vercel_line_events(line: &str) -> Vec<StreamEvent> {
    ...
    parse_vercel_sse(payload).into_iter().collect()
}
